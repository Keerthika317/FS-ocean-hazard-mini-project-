...
const sendReportToAdmin = async (reportData, photoUrl) => {
    ...
    try {
        const severityIcon = { 'Low': '🟢', 'Medium': '🟡', 'High': '🟠', 'Critical': '🔴' }[reportData.severity] || 'Error';
...
const insertPredefinedHazardsIntoDB = async () => {
    for (const hazard of predefinedHazardsList) {
        if (insertedHazardIds.has(hazard.id)) continue;
        
        const [existing] = await db.promise().query(
            'SELECT id FROM social_alerts WHERE content = ? LIMIT 1',
            [hazard.title]
        );
         ...
         if ((existing as any).length === 0) {
            ...
            await db.promise().query(
                ...
                ['Global Monitor', 'Alert System', hazard.title, hazard.location, 'CRITICAL', hazard.hazard, randomDate]
            );
            ...
            try {
                ...
                if (response.data && response.data.articles) {
                    for (const article of response.data.articles) {
                        ...
                        if (processedArticleHashes.has(titleHash)) continue;
                        
                        const [existing] = await db.promise().query(
                            `SELECT id FROM social_alerts WHERE LOWER(content) LIKE ? LIMIT 1`,
                            [`%${titleHash.substring(0, 50)}%`]
                        );
                       if ((existing as any).length > 0) {
                        ...
                        await db.promise().query(
                            'INSERT INTO social_alerts (platform, username, content, location, sentiment, hazard_detected, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)',
                            ['GNews', article.source?.name || 'News', cleanTitle, randomLocation, 'CRITICAL', hazard.display, randomDate]
                        );
                        newAlertsAdded++;
...
const autoArchiveResolved = async () => {
    try {
        ...
        for (const hazard of (resolvedHazards as any)) {
            await db.promise().query(
...
app.post('/api/restore-hazard/:id', (req, res) => {
    ...
    db.query('SELECT * FROM hazard_archive WHERE id = ?', [archiveId], (err, archived) => {
        if (!archived || (archived as any).length === 0) { 
    return res.status(404).json({ error: "Not found" });
}
        const hazard = (archived as any)[0];
        db.query(
...
app.post('/api/archive-hazard/:id', async (req, res) => {
    ...
    db.query('SELECT * FROM hazard_reports WHERE id = ?', [hazardId], async (err, hazards) => {
        if (err || (hazards as any).length === 0) return res.status(404).json({ error: "Hazard not found" });
...
app.post('/api/reports', upload.single('photo'), async (req, res) => {
    ...
    db.query(sql, [reporter_name, reporter_email, hazard_type, location, severity, 13.0827, 80.2707, photo_url, validImpactRadius, validRiskCount, user_id], async (err, result) => {
        ...
        const reportData = { 
            id: (result as any).insertId, reporter_name, reporter_email, hazard_type, location, 
...
app.patch('/api/reports/:id/status', async (req, res) => {
    ...
    db.query('SELECT * FROM hazard_reports WHERE id = ?', [reportId], async (err, reports) => {
        if (err) return res.status(500).json({ error: err.message });
       if (!reports || (reports as any).length === 0)  return res.status(404).json({ error: "Report not found" });
...
app.get('/api/export-csv', (req, res) => {
    db.query('SELECT * FROM hazard_reports', (err, results) => {
        ...
        const rows = (results as any).map((r: any) => `${r.id},${r.reporter_name},${r.reporter_email || ''},${r.hazard_type},${r.location},${r.severity},${r.impact_radius},${r.risk_count},${r.status},${r.created_at}`).join("\n");
